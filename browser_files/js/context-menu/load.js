module.exports = function (SOCIALBROWSER) {
  ...
  if (!SOCIALBROWSER.user_agent_info) {
    ...
    if (SOCIALBROWSER.user_agent_info) {
      if (SOCIALBROWSER.user_agent_info.vendor) {
        ...
        if (SOCIALBROWSER.user_agent_info.engine.name === 'Chrome') {
        ...
        } else if (SOCIALBROWSER.user_agent_info.engine.name === 'Edge') {
          let chrome = JSON.parse(
          ...
          chrome.loadTimes = () => {};
          SOCIALBROWSER.__define(window, 'chrome', chrome);
        } else if (SOCIALBROWSER.user_agent_info.engine.name === 'Firefox') {
  ...
  SOCIALBROWSER.on('$download_item', (e, dl) => {
    SOCIALBROWSER.showDownloads(` ${dl.status} ${(dl.received / dl.total).toFixed(2) * 100} %  ${dl.name} ( ${(dl.received / 1000000).toFixed(2)} MB / ${(dl.total / 1000000).toFixed(2)} MB )`);
  });
};
